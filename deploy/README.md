...
`systemctl status` on the target shows it already runs two other full app
stacks plus a background transcription service, all on 2 vCPUs / 3.8GB RAM,
sharing one Postgres 16 cluster (`postgresql@16-main`) across apps. Ports
3000 and 3001 are already taken by those other services — politikkoll uses
**3002**.
...
the other apps' already-running Postgres too. If you want tuning help,
share the current `postgresql.conf` first rather than applying generic
advice sized for a dedicated instance.
...
2. **pgvector for the existing Postgres 16.** Confirm whether it's already
   installed (the other apps on this box may not use it) — `psql -c "\dx"`
   in any existing database. If not: `apt install postgresql-16-pgvector`, or
...
5. **Clone the app** into `/var/www/politikkoll` — this box keeps app code
   ...
   reserved for tooling like rbenv and other apps' own runtime environments).
